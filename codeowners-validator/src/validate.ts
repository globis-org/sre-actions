...
import { parseCodeOwners, getFileOwners } from './parse'
...
export const validateCodeOwners = async (inputs: Inputs) => {
  ...
  const fileOwners = getFileOwners(
    filenames,
    parseCodeOwners(inputs.codeowners)
  )
  core.debug(`File owners:\n${JSON.stringify(fileOwners, null, 2)}`)

  // Extract members from teams
  const fileOwnerUsersPromise = fileOwners.map(async owner => {
    if (owner.kind === 'user') {
      return owner.name
    } else {
  ...
  const fileOwnerUsers = await Promise.all(fileOwnerUsersPromise).then(
    members => members.flat()
  )
  core.info(`File codeowners: ${fileOwnerUsers.join(', ')}`)
  ...
  const approverOwners = approvers.filter(approver =>
    fileOwnerUsers.includes(approver)
  )

  const sha = (context.payload.pull_request as PullRequestPayload).head.sha

  if (fileOwnerUsers.length === 0 || approverOwners.length > 0) {
    await octokit.rest.repos.createCommitStatus({
      owner: context.repo.owner,
      repo: context.repo.repo,
      sha,
      state: 'success',
      context: CommitContext,
      description: `Approved by ${approverOwners.join(', ')}.`,
    })
  } else {
    await octokit.rest.repos.createCommitStatus({
      owner: context.repo.owner,
