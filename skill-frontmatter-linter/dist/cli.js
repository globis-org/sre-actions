...
function loadConfig(configPath) {
  ...
  const raw = (0, import_yaml.parse)((0, import_node_fs.readFileSync)(configPath, "utf-8"));
  if (!raw) {
    return { ...DEFAULT_CONFIG };
  }
  ...
  if (Array.isArray(discovery["claudeSkills"]))
    cfg.claudeSkillsDirs = discovery["claudeSkills"];
  if (Array.isArray(discovery["claudeCommands"]))
    cfg.claudeCommandsDirs = discovery["claudeCommands"];
  if (Array.isArray(discovery["pluginRoots"]))
    cfg.pluginRoots = discovery["pluginRoots"];
  const fm = getSection(raw, "frontmatter");
  if (Array.isArray(fm["required"])) cfg.requiredFields = fm["required"];
  if (Array.isArray(fm["skillRequired"])) cfg.skillRequiredFields = fm["skillRequired"];
  ...
  if (Array.isArray(tools["allowed"])) cfg.allowedTools = tools["allowed"];
  if (Array.isArray(tools["requireJustification"]))
    cfg.requireJustification = tools["requireJustification"];
  return cfg;
}

// src/checker.ts
var BARE_BASH_RE = /(?:^|,)\s*Bash\s*(?:,|$)/;
var MCP_FUNC_RE = /(?:^|,)\s*MCP\(/;
var BASH_SPEC_RE = /Bash\(([^)]*)\)/g;
...
function discoverFiles(root, cfg) {
  ...
  for (const dir of cfg.claudeSkillsDirs) {
    const base = (0, import_node_path.join)(root, dir);
    if (!isDir(base)) continue;
    for (const entry of sortedDirEntries(base)) {
      const skillFile = (0, import_node_path.join)(base, entry, "SKILL.md");
      if (isDir((0, import_node_path.join)(base, entry)) && (0, import_node_fs2.existsSync)(skillFile)) {
  ...
  for (const pluginRoot of cfg.pluginRoots) {
    const base = (0, import_node_path.join)(root, pluginRoot);
    if (!isDir(base)) continue;
    for (const pluginName of sortedDirEntries(base)) {
      const pluginDir = (0, import_node_path.join)(base, pluginName);
      if (!isDir(pluginDir)) continue;
      const skillsBase = (0, import_node_path.join)(pluginDir, "skills");
      if (isDir(skillsBase)) {
        for (const skillName of sortedDirEntries(skillsBase)) {
          const skillFile = (0, import_node_path.join)(skillsBase, skillName, "SKILL.md");
          if (isDir((0, import_node_path.join)(skillsBase, skillName)) && (0, import_node_fs2.existsSync)(skillFile)) {
            targets.push({ path: skillFile, kind: "skill" });
          }
        }
      }
      const commandsBase = (0, import_node_path.join)(pluginDir, "commands");
...
function collectCommandFiles(dir, targets) {
...
function parseFrontmatter(content) {
  ...
  let parsed;
  try {
    parsed = (0, import_yaml2.parse)(fmText);
  } catch (e) {
  ...
  return { fm: parsed, endLine: endIdx + 1 };
}
function extractToolNames(toolsStr) {
  const names = [];
  let rest = toolsStr;
  while (rest.length > 0) {
    rest = rest.replace(/^[\s,]+/, "");
...
function checkAllowedToolsSpecifiers(toolsStr, relPath) {
  const findings = [];
...
function checkUnclosedCodeFences(content, relPath) {
  const findings = [];
  const lines = content.split("\n");
  let bodyStart = 0;
  if (lines.length && lines[0].trim() === "---") {
...
function normalizeAllowedTools(value, relPath, findings) {
  if (Array.isArray(value)) {
  ...
  if (str.includes("\n")) {
    findings.push({
      ...
      message: "block scalar (|) in allowed-tools is not officially supported; use a space/comma-separated string or a YAML list"
    });
  }
  return normalizeSpaceSeparated(str);
}
function normalizeSpaceSeparated(str) {
...
function checkFile(target, root, cfg) {
  const findings = [];
  const relPath = (0, import_node_path.relative)(root, target.path);
  let content;
  try {
    content = (0, import_node_fs2.readFileSync)(target.path, "utf-8");
  } catch (e) {
    findings.push({
      path: relPath,
      line: 1,
  ...
  if (target.kind === "skill") {
    ...
    const skillDirName = (0, import_node_path.basename)((0, import_node_path.dirname)(target.path));
    if (rawName !== void 0 && rawName !== null && rawName !== "") {
  ...
  if (allowedToolsRaw) {
    ...
    for (const tool of cfg.requireJustification) {
      ...
      if (toolNames.includes(tool)) {
        ...
        if (typeof description === "string" && !description.toLowerCase().includes(tool.toLowerCase())) {
          findings.push({
            path: relPath,
            line: 1,
            level: "warning",
...
function formatText(allFindings) {
...
function escapeGitHubAnnotation(s) {
  return s.replaceAll("%", "%25").replaceAll("\r", "%0D").replaceAll("\n", "%0A");
}
function formatGitHub(allFindings) {
  const lines = [];
  for (const findings of allFindings.values()) {
    for (const f of findings) {
...
function formatJson(allFindings) {
  const output = {};
  for (const [key, findings] of allFindings) {
    output[key] = findings.filter((f) => f.level !== "pass").map((f) => ({
      path: f.path,
      line: f.line,
      level: f.level,
      title: f.title,
...
function run(options) {
  const root = (0, import_node_path.resolve)(options.path);
  const configPath = (0, import_node_path.resolve)(root, options.config);
  ...
  for (const target of targets) {
    const key = target.kind === "skill" ? (0, import_node_path.relative)(root, (0, import_node_path.dirname)(target.path)) : (0, import_node_path.relative)(root, target.path);
...
function sortedDirEntries(dir) {
  return (0, import_node_fs2.readdirSync)(dir).toSorted();
}
