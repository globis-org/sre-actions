...
var generateTag = async (env, type) => {
  const { ref, sha } = import_github.context;
  const { stdout } = await (0, import_exec.getExecOutput)("date", ["+%Y%m%d-%H%M%S"]);
  const timestamp = stdout.trim();
  const imageTag = env ? `${env}-${timestamp}-${suffix(type, ref, sha)}` : `${timestamp}-${suffix(type, ref, sha)}`;
